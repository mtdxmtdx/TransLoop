...
按截图快捷键后拖拽框选屏幕区域，松开鼠标后进入截图翻译结果窗。结果窗左侧为原文，右侧为译文；原文可手动编辑后重新翻译，目标语言也可在结果窗中切换。截图翻译始终使用结果窗或设置中明确选择的目标语言，不会被智能语言方向自动改写。
...
- **视觉输出净化**：改进结构化结果解析，拒绝模型回显系统提示词，并区分视觉翻译与纯 OCR 的响应格式。
- **设置同步**：截图和悬浮窗读取设置前刷新共享配置，避免语言切换后继续使用旧值。
- **版本显示**：隐私政策弹窗动态显示当前应用版本。

### 1.0.3 · 2026-08-23 — DeepSeek 视觉模型支持与体验优化

- **DeepSeek 多模态模型支持**：支持 DeepSeek 官方视觉模型 `deepseek-v4-flash-vision-exp` 用于截图 OCR 与图片翻译。
- **模式 A 体验优化**：当选择 DeepSeek 且模型为 `deepseek-v4-flash-vision-exp` 时，自动取消不支持图片输入的警告。
- **OCR 识别模型支持**：多模型协作识别提供方列表中新增 DeepSeek，支持一键配置并预填 `deepseek-v4-flash-vision-exp`。
- **用量估算与原生网关**：Rust 原生请求网关同步放行 DeepSeek 多模态请求，并补充用量费用估算配置。

### 1.0.2 · 2026-07-17 — 修复与频率策略更新

- **截图翻译修复**：允许主窗口、划词悬浮窗和截图窗口安全触发旧凭证迁移，修复截图翻译误报迁移失败的问题。
- **本地限流调整**：普通文本翻译每 1 秒最多 1 次，截图视觉翻译 / OCR 每 2 秒最多 1 次，Provider 测试每 5 秒最多 1 次。
- **限流上限同步**：对应每个 Provider 与操作分别为 60、30、12 次/分钟，并保留全局最多 2 个并发请求。
- **回归验证**：新增本地频率策略和即时重复请求拦截测试。
