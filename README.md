...
- **框选**：全屏截图覆盖到当前显示器，橡皮筋拖拽选区（实时尺寸显示），Esc 或右键取消。
- **OCR 模式 A（多模态）**：图像直接发给 GPT-4o / Qwen3-VL-Flash / Grok 等视觉模型，建议配合多模型协作（一个模型 OCR，另一个模型翻译）。
- **OCR 模式 B（Windows 离线）**：调用 Windows.Media.Ocr 系统引擎识别文字，再走翻译模型。
- **多模型协作**（仅模式 A）：开启后先由「识别模型」做纯 OCR，再由「翻译提供方」翻译。可将 Qwen3-VL-Flash 等视觉模型与 DeepSeek 等文本模型组合使用。
